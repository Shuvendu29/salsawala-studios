      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            <div className="flex flex-col leading-none mb-4">
              <span className="font-display text-2xl font-bold text-white">SALSAWALA</span>
              <span className="font-body text-[9px] tracking-[0.3em] text-gold uppercase">Studios • Kolkata</span>
            </div>
            ...
            <div className="flex gap-3">
              {socials.map(({ icon: Icon, href, label }) => (
                <a
                  ...
                  aria-label={label}
                  className="h-9 w-9 rounded-full bg-dark-surface border border-dark-border flex items-center justify-center text-white/60 hover:text-white hover:border-primary hover:bg-primary/10 transition-all"
                >
          ...
          {/* Quick Links */}
          <div>
            <h3 className="font-display text-white font-semibold mb-5 text-sm tracking-widest uppercase">
              Quick Links
            </h3>
            <ul className="space-y-3">
              {quickLinks.map(link => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-white/50 hover:text-primary text-sm font-body transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
          ...
          <div>
            ...
            <ul className="space-y-4">
              ...
              <li className="flex items-center gap-3">
                ...
                <a href="tel:+919830158223" className="text-white/50 hover:text-primary text-sm font-body transition-colors">
                  +91 98301 58223
                </a>
              </li>
              <li className="flex items-center gap-3">
                <Mail className="h-4 w-4 text-primary shrink-0" />
                <a href="mailto:salsawalastudios@gmail.com" className="text-white/50 hover:text-primary text-sm font-body transition-colors">
                  salsawalastudios@gmail.com
                </a>
              </li>
            </ul>
            <div className="mt-6 p-3 bg-dark-surface border border-dark-border rounded-xl">
              <p className="text-xs text-white/40 font-body">Studio Hours</p>
              <p className="text-sm text-white/70 font-body mt-1">Mon–Fri: 5:30 PM – 10:00 PM</p>
              <p className="text-sm text-white/70 font-body">Sat–Sun: 9:00 AM – 10:00 PM</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 pt-6 border-t border-dark-border flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-white/30 text-xs font-body flex items-center gap-1">
            Made with <Heart className="h-3 w-3 text-primary fill-primary" /> in Kolkata
          </p>
        </div>
      </div>
    </footer>
