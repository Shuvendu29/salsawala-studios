      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="font-body text-white/50 mt-1">Salsawala Studios Control Panel</p>
          </div>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: 'Total Students', value: '512', sub: '+24 this month', icon: Users, color: 'text-primary', bg: 'bg-primary/10 border-primary/20' },
                { label: 'Active Classes', value: `${weeklySchedule.length}`, sub: 'This week', icon: BookOpen, color: 'text-gold', bg: 'bg-gold/10 border-gold/20' },
                { label: 'Monthly Revenue', value: '₹1.2L', sub: '+18% vs last month', icon: DollarSign, color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20' },
                { label: 'Upcoming Events', value: `${upcomingEvents.length}`, sub: 'Next 60 days', icon: Calendar, color: 'text-purple-400', bg: 'bg-purple-500/10 border-purple-500/20' },
                ...
                <Card key={label} className="p-5">
                  ...
                  <div className="font-body text-xs text-white mt-0.5 font-medium">{label}</div>
                  <div className="font-body text-xs text-white/30">{sub}</div>
                </Card>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="p-6">
                <div className="flex items-center justify-between mb-5">
                  ...
                  <button className="text-xs text-primary font-body hover:text-primary-light flex items-center gap-1">
                  ...
                  {recentBookings.map((b, i) => (
                    <div key={i} className="flex items-center gap-3 p-3 bg-dark-surface rounded-xl border border-dark-border">
                      <div className="h-8 w-8 rounded-full bg-primary/20 flex items-center justify-center shrink-0">
                        <span className="font-display text-xs font-bold text-primary">{b.name.charAt(0)}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-body text-sm text-white truncate">{b.name}</p>
                        <p className="font-body text-xs text-white/40">{b.class} · {b.time}</p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="font-display text-sm font-bold text-white">₹{b.amount}</p>
              ...
              <div className="space-y-4">
                <Card className="p-6">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      { label: 'Add Class', icon: Plus, color: 'text-primary', bg: 'bg-primary/10 border-primary/20', action: () => toast.success('Open class creation modal') },
                      { label: 'Add Event', icon: Calendar, color: 'text-gold', bg: 'bg-gold/10 border-gold/20', action: () => toast.success('Open event creation modal') },
                      { label: 'Add Faculty', icon: Users, color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20', action: () => toast.success('Open faculty creation modal') },
                      { label: 'Site Settings', icon: Settings, color: 'text-purple-400', bg: 'bg-purple-500/10 border-purple-500/20', action: () => toast.success('Open settings') },
                      ...
                      >
                        ...
                        <span className="font-body text-xs text-white/60 group-hover:text-white transition-colors">{label}</span>
                      </button>
                    ))}
                  </div>
                </Card>
        ...
        {activeTab === 'classes' && (
          <div>
            <div className="flex justify-between items-center mb-6">
              <h2 className="font-display text-xl font-bold text-white">All Classes</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {weeklySchedule.map(cls => (
                <Card key={cls.id} className="p-4 hover:border-primary/30 transition-all">
                  ...
                  <p className="font-body text-sm text-white/50 mb-3">{cls.instructor}</p>
                  <div className="flex gap-4 text-xs text-white/40 font-body mb-3">
                    <span>{cls.day}</span>
                    <span>{cls.time}</span>
                    <span>{cls.spots}/{cls.totalSpots} spots</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="font-display font-bold text-white">₹{cls.price}</span>
        ...
        {activeTab === 'events' && (
          <div>
            <div className="flex justify-between items-center mb-6">
              <h2 className="font-display text-xl font-bold text-white">All Events</h2>
                ...
                return (
                  <Card key={event.id} className="overflow-hidden hover:border-primary/30 transition-all">
                    ...
                    <div className="p-4">
                      ...
                      <p className="font-body text-xs text-white/50 mb-3">{event.time} · {event.venue}</p>
                      <div className="flex justify-between items-center">
                        <div>
                          <span className="font-display font-bold text-white">₹{event.price}</span>
                          <span className="text-xs text-white/40 font-body ml-1">{event.enrolled}/{event.capacity} registered</span>
                        </div>
                        <div className="flex gap-2">
                          <Button size="sm" variant="secondary">Edit</Button>
                        </div>
        ...
        {activeTab === 'analytics' && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <Card key={stat.label} className="p-5 text-center">
                  ...
                  <div className="font-body text-xs text-white/30">{stat.sub}</div>
                </Card>
              ))}
            </div>

            <Card className="p-6">
              <h3 className="font-display text-lg font-bold text-white mb-4">Style Popularity</h3>
              <div className="space-y-3">
                {[
                  ...
                  <div key={item.style} className="flex items-center gap-3">
                    <span className="font-body text-sm text-white/60 w-28 shrink-0">{item.style}</span>
                    <div className="flex-1 bg-dark-surface rounded-full h-2 overflow-hidden">
                      <div className="h-full bg-crimson-gradient rounded-full" style={{ width: `${item.pct}%` }} />
                    </div>
                    <span className="font-body text-xs text-white/40 w-16 text-right shrink-0">{item.count} students</span>
                  </div>
                ))}
              </div>
            </Card>
            ...
            <Card className="p-6">
              <h3 className="font-display text-lg font-bold text-white mb-2">Firebase Analytics</h3>
